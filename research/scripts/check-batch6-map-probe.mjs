...
const behavioralChunk = (pathname) => /buyerConcierge|leadAttributionStore|conciergeLauncher/.test(pathname);
const server = http.createServer(async (req, res) => {
  try {
    const pathname = decodeURIComponent(new URL(req.url, "http://localhost").pathname);
    const trace = behavioralChunk(pathname);
    const received = Date.now();
    if (trace) console.log(`map-probe server.receive ${pathname}`);
    ...
    res.end(body, () => {
      if (trace) console.log(`map-probe server.sent ${pathname} ${Date.now() - received}ms ${body.length}b`);
    });
  } catch {
    res.writeHead(404);
    res.end("Not found");
...
try {
  ...
  page.on("pageerror", (error) => { pageErrors.push(error.message); console.log(`map-probe pageerror ${error.message}`); });
  ...
  const relevant = (url) => behavioralChunk(url);
  ...
  await step("application.shell", () => page.waitForFunction(() => { const app = document.querySelector("#app"); return Boolean(app && app.childElementCount > 0); }, null, { timeout: 10000 }), 12000);
  ...
  postMountState = await step("dialog.post-mount-state", () => page.evaluate(() => {
    ...
    return { root: describe(root), launcher: describe(launcher), panel: describe(panel), launcherExpanded: launcher?.getAttribute("aria-expanded") ?? null, activeTag: document.activeElement?.tagName ?? null, activeClass: document.activeElement?.getAttribute("class") ?? null };
  }), 5000);
  ...
  for (const heading of ["Research", "Current information", "Talk to the team"]) assert.equal(await dialog.getByRole("heading", { name: heading }).count(), 1, `/map/: ${heading}`);
  ...
  failure = error;
  const domState = page && !page.isClosed() ? await Promise.race([
    page.evaluate(() => ({ launcherCount: document.querySelectorAll(".buyer-concierge-launcher").length, rootCount: document.querySelectorAll("[data-buyer-concierge-root]").length, panelCount: document.querySelectorAll("[data-buyer-concierge-panel]").length, launcherExpanded: document.querySelector(".buyer-concierge-launcher")?.getAttribute("aria-expanded") ?? null, bodyChildren: document.body.childElementCount })).catch(() => null),
    new Promise((resolve) => setTimeout(() => resolve({ diagnosticTimedOut: true }), 2000)),
  ]) : null;

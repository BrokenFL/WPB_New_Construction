...
const EXPECTED={
 ...
 'mandarin-oriental':{delivery:'To confirm \u2014 completion estimates vary across reports.',price:'From $3.5M (reported; request current pricing)',address:'5400 N Flagler Drive, West Palm Beach, FL 33480',floors:'31',compareDelivery:'To confirm \u2014 reports vary',comparePrice:'From $3.5M reported; request current pricing'},
...
test('mandarin-oriental: ZIP corrected, no 2031 anywhere in route metadata or copy',()=>{
 const h=html('/projects/mandarin-oriental/');
 const c=copies.find(c=>c.repoProjectId==='mandarin-oriental');
 assert.ok(h.includes('33480'),'corrected ZIP present');
 assert.doesNotMatch(h,/33407/,'stale ZIP gone');

...
async function publishNewsArticle({ sourceFile, normalized, articleId, routeSlug, input, existing }) {
  ...
  const item = {
    ...
    titleTag: clean(input.titleTag || existing?.titleTag || ""),
    metaDescription: clean(input.metaDescription || existing?.metaDescription || input.description || existing?.description || normalized.article.deck),
    deck: normalized.article.deck,
    ...
    bodySections: normalized.bodySections,
    imageAlt: clean(normalized.heroImage?.alt || existing?.imageAlt || ""),
    imageCaption: clean(normalized.heroImage?.caption || existing?.imageCaption || ""),
    imageCredit: clean(normalized.heroImage?.credit || existing?.imageCredit || ""),

...
export function renderCommercialDocument(source, page) {
  ...
  html = html.replace(introPattern, (_, open) => `${open}<section class="cg-static-intro"><p>WPB New Construction</p><h1>${e(copy.heading)}</h1><p>${e(copy.intro)}</p>${renderCommercialActions(page)}</section>${page === 'home' ? renderCommercialGuide(page) : ''}`);
  if (page === 'buildings') {
    if ((html.match(/<\/main>/g) ?? []).length !== 1) throw new Error('Expected one directory main landmark');
    html = html.replace('</main>', `${renderCommercialGuide(page)}</main>`);
  }

...
import { homepageProjectCardImage } from '../data/homepageAssets.ts';
...
type ProjectGuide = { name: string; slug: string; stage: string; detail: string; verify: string; source: Link; image: string; imageAlt: string };
...
const approvedProjectCardImage = (projectId:string): string => homepageProjectCardImage(projectId) ?? '';
export const corridorGrowthPages: Record<GrowthCorridor, CorridorCopy> = {
  ...
  'south-flagler': {
    ...
    resources:[{label:'South Flagler House released floor plans',href:'/floorplans/#floorplans-south-flagler-house'},{label:'Compare South Flagler House with La Clara',href:'/compare/?projects=south-flagler-house,la-clara'},{label:'North Flagler vs South Flagler',href:'/answers/north-flagler-vs-south-flagler-new-condos/'},{label:'Preconstruction vs completed residences',href:'/answers/preconstruction-vs-completed-new-construction-condos-west-palm-beach/'},{label:'Compare buildings',href:'/compare/'},{label:'Browse released floor plans',href:'/floorplans/'}],
  },
  'palm-beach':{
...
function renderCorridorExploreActions(key:GrowthCorridor): string {
  const c=corridorGrowthPages[key];
  return `<div class="cg-actions cr-actions cr-explore-actions"><a class="cg-button" href="#cr-project-heading-${key}">Explore ${c.projects.length} ${e(c.label)} projects</a><a class="cg-button cg-secondary" href="/buildings/">Browse all buildings</a></div>`;
}
...
export function renderGrowthCorridor(key:GrowthCorridor): string {
  const c=corridorGrowthPages[key];
  const projectCards=c.projects.map(p=>`<article class="cr-project" data-project-slug="${e(p.slug)}"><figure class="cr-project-media"><img src="${e(p.image)}" alt="${e(p.imageAlt)}" loading="lazy" decoding="async" width="1200" height="800"></figure><div class="cr-project-body"><p class="cr-stage">${e(p.stage)}</p><h3><a href="/projects/${e(p.slug)}/">${e(p.name)}</a></h3><p>${e(p.detail)}</p><p><strong>Before shortlisting:</strong> ${e(p.verify)}</p><div class="cr-project-footer"><a href="/projects/${e(p.slug)}/">Explore building guide <span aria-hidden="true">→</span></a><a class="cr-source" href="${e(p.source.href)}" rel="noopener noreferrer">${e(p.source.label)} <span aria-hidden="true">↗</span></a></div></div></article>`).join('');
  return `<div class="cr-page" data-corridor-growth="${key}"><section class="cr-hero"><div><nav class="cr-crumbs" aria-label="Breadcrumb"><a href="/">Home</a> / <a href="/corridors/">Corridors</a> / <span>${e(c.label)}</span></nav><p class="eyebrow">Corridor Guide</p><h1>${e(c.heading)}</h1><p class="cr-intro">${e(c.intro)}</p>${renderCorridorExploreActions(key)}</div><figure><img src="${e(c.image)}" alt="${e(c.imageAlt)}" decoding="async" fetchpriority="high" width="1000" height="750"><figcaption>${e(c.imageCaption)}</figcaption></figure></section>
  <section class="cr-geography"><h2>Know the location you are comparing.</h2><p>${e(c.geography)}</p></section>
  <section class="cr-projects" aria-labelledby="cr-project-heading-${key}"><div class="cr-section-heading"><p class="eyebrow">Start your shortlist</p><h2 id="cr-project-heading-${key}">Project position, not live inventory.</h2><p>Sales and construction are separate dimensions. Sources checked <time datetime="${corridorReviewedDate}">September 7, 2026</time>; historical milestones are identified below.</p></div><div class="cr-project-grid">${projectCards}</div></section>
  <section class="cr-contact" aria-label="Request current ${e(c.label)} details"><div><p class="eyebrow">Next step</p><h2>Build a focused ${e(c.label)} shortlist.</h2><p>Request current availability or the latest pricing and floor-plan packet after comparing the project guides.</p></div><div>${renderCorridorActions(key)}<p class="cr-note">Requests go to The Scott Gordon Group. Availability and packet contents require current confirmation; no instant inventory or delivery promise.</p></div></section>

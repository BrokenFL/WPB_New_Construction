...
// The existing gatekeeper keeps selected developer destinations off public
// outbound navigation. Preserve their real source names and schema citations,
// without disguising an external link or relaxing the shared restriction.
const outboundSourceKeys = new Set<SourceKey>(['olara', 'olaraLife', 'olaraBuild', 'ritz', 'ritzLife', 'ritzBuild', 'southBuild', 'clara']);
function sourceCitation(key: SourceKey): string {
  const source = sources[key];
  return outboundSourceKeys.has(key)

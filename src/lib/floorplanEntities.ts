  ...
  return `<div class="fp-page" data-floorplan-id="${e(plan.planId)}">
    ...
    <main id="floorplan-main" class="fp-main">
      ...
      <div class="fp-layout"><section aria-labelledby="fp-drawing-title"><h2 id="fp-drawing-title">The released drawing</h2>
        ...
        <div class="fp-actions"><a class="fp-button" href="${plan.pdf}" data-fp-action="pdf" download>Download floor plan PDF</a><a href="${plan.sourceUrl}" data-fp-action="source" target="_blank" rel="noopener noreferrer">View developer source PDF ↗</a></div>
      </section><aside class="fp-facts" aria-labelledby="fp-facts-title"><h2 id="fp-facts-title">Plan at a glance</h2><dl>${facts.map(([label, value]) => `<div><dt>${e(label)}</dt><dd>${e(value)}</dd></div>`).join("")}</dl>${plan.areaNote ? `<p class="fp-area-note"><strong>Area clarification:</strong> ${e(plan.areaNote)}</p>` : ""}<p>Reported areas include different types of space. The total is not the interior living area.</p><a class="fp-button" href="/inquire/" data-fp-action="availability">Request current availability</a><p class="fp-small">Ask about ${e(fullName(plan))}, the current drawing, pricing and available floors.</p></aside></div>
      ...
      <section class="fp-source"><h2>Source and review notes</h2><p>${e(plan.sourceNote)}</p><p>Facts on this page were checked against the linked developer drawing on <time datetime="${plan.reviewedOn}">${plan.reviewedOn}</time>. <a href="${plan.sourcePage}" data-fp-action="source" target="_blank" rel="noopener noreferrer">Visit the developer’s residence information ↗</a></p></section>

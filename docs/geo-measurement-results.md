...
Post-deploy Search Console check on 2026-06-04: the submitted sitemap row still shows `https://www.wpbnewconstruction.com/sitemap.xml` with Status `Success`, Submitted May 26, 2026, Last read Jun 3, 2026, 57 discovered pages, and 0 discovered videos. Page Indexing shows 39 indexed pages and 233 not indexed pages; the not-indexed reasons shown were alternate page with proper canonical tag, page with redirect, not found (404), discovered - currently not indexed, and crawled - currently not indexed. HTTPS shows 0 non-HTTPS URLs and 9 HTTPS URLs. Core Web Vitals shows not enough usage data for mobile or desktop. Breadcrumbs shows 5 valid and 0 invalid items. FAQ shows 2 valid and 0 invalid items, with Google's notice that FAQ rich results are being deprecated. Links still says processing data.

URL Inspection follow-up on 2026-06-04: the inspection bar accepted the updated Rosewood and Rybovich URLs and showed exact autocomplete rows, but selecting the rows returned to Overview instead of opening URL Inspection results. No indexing request was submitted because the inspection result screen and request-indexing control were not reached.

...
- The Builder now includes a Project Intelligence Review cockpit and a Project Facts direct-update panel for resolving compare/source/schema conflicts before they reach JSON-LD. Manual overrides are written to `content/overrides/project-fact-overrides.json`. A field only reaches JSON-LD when its override has `schemaSafe: true`, which Brooke sets explicitly in the Builder.
